...
void *gps_task(void *arg)
{
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		...
		if(network)
		{
			pthread_mutex_lock(&(s->network.lock));
			if(s->network.status == NO_CONNECTION)
				pthread_cond_wait(&(s->network.network_ok), &(s->network.lock));
			xyz[iumd][0] = s->network.x;
			xyz[iumd][1] = s->network.y;
			xyz[iumd][2] = s->network.z;
			if(s->network.status == STOPPED)
			{
				pthread_mutex_unlock(&(s->network.lock));
				goto abort;
			}
			pthread_mutex_unlock(&(s->network.lock));
		}
