...
void init_sim(sim_t *s)
{
	...
	pthread_mutex_init(&(s->network.lock), NULL);
	s->network.status = NO_CONNECTION;
	s->network.x = s->network.y = s->network.z = 0.0f;
	pthread_cond_init(&(s->network.network_ok), NULL);
...
{
	sock_t* sock_arg = (sock_t*) arg;
    int sock = sock_arg->sock;
    int read_size;
    char *message , client_message[513];
	char command[32];
    while( (read_size = recv(sock , client_message, 512 , 0)) > 0 )
    {
		...
		pthread_mutex_lock(&(sock_arg->s->network.lock));
		if(strcmp(command, "ECEF") == 0)
		{
			double x, y, z;
			x = y = z = 0.0f;
			sscanf(client_message + strlen(command), "%lf%lf%lf", &x, &y, &z);
			if(x != 0.0f)
			{
				printf("From Network: %lf,%lf,%lf\n", x, y, z);
				sock_arg->s->network.x = x;
				sock_arg->s->network.y = y;
				sock_arg->s->network.z = z;
				sock_arg->s->network.status = CONNECTED;
				pthread_cond_signal(&(sock_arg->s->network.network_ok));
			}
			pthread_mutex_unlock(&(sock_arg->s->network.lock));
		}
		// else if(strcmp(command, "STOP") == 0)
		// {
		// 	puts("Stop!\n");
		// 	sock_arg->s->network.status = STOPPED;
		// 	pthread_mutex_unlock(&(sock_arg->s->network.lock));
		// }
		// else if(strcmp(command, "START") == 0 && sock_arg->s->network.status == STOPPED)
		// {
		// 	puts("Start!\n");
		// 	sock_arg->s->network.status = NO_CONNECTION;
		// 	pthread_mutex_unlock(&(sock_arg->s->network.lock));
		// 	restart_sim(sock_arg->s);
		// 	start_task(sock_arg->s);
...
void *network_task(void *arg)
{
    ...
    server.sin_port = htons(s->opt.networkPort);
...
int start_network_task(sim_t *s)
{
	int status;

	status = pthread_create(&(s->network.thread), NULL, network_task, s);
...
void usage(char *progname)
{
	printf("Usage: %s [options]\n"
		...
		"  -p <port>    	Server mode (listen on <port>)\n"
...
void interrupt_handler (int signum) {
	if(s.opt.network && (signum == SIGINT || signum == SIGTERM))
	{
		puts("SIGINT closing socket...");
...
int main(int argc, char *argv[])
{
	...
	while ((result=getopt(argc,argv,"e:u:g:l:p:T:t:d:a:iI"))!=-1)
	{
		switch (result)
		{
		case 'e':
		...
		case 'p':
			...
			sscanf(optarg,"%d",&s.opt.networkPort);
			break;
		case 'T':
	...
	if(s.opt.network)
	{
		pthread_join(s.network.thread, NULL);
	}
	else
	{
		pthread_join(s.tx.thread, NULL);	
